/**
 * CodemarieAgent - Decoupled ACP Agent implementation for Codemarie CLI.
 *
 * This class implements the ACP (Agent Client Protocol) Agent interface,
 * allowing Codemarie to be used programmatically without stdio dependency.
...
import type { CodemarieMessageChange } from "@core/task/message-state"
...
import { CodemarieSessionEmitter } from "./CodemarieSessionEmitter.js"
import { translateMessage } from "./messageTranslator.js"
import { handlePermissionResponse } from "./permissionHandler.js"
import type { CodemarieAcpSession, CodemarieAgentOptions, PermissionHandler } from "./public-types.js"
...
/**
 * Codemarie's implementation of the ACP Agent interface.
 *
 * This agent bridges the ACP protocol with Codemarie's core Controller,
 ...
 * For stdio-based ACP communication, use the AcpAgent wrapper class.
 */
export class CodemarieAgent implements acp.Agent {
	private readonly options: CodemarieAgentOptions
	private readonly ctx: CliContextResult

	/** Map of active sessions by session ID */
	public readonly sessions: Map<string, CodemarieAcpSession> = new Map()

	/** WeakMap to associate CodemarieAcpSession with its Controller without exposing it to consumers */
	readonly #sessionControllers = new WeakMap<CodemarieAcpSession, Controller>()
	...
	private readonly sessionEmitters: Map<string, CodemarieSessionEmitter> = new Map()
	...
	constructor(options: CodemarieAgentOptions) {
		this.options = options
		this.ctx = initializeCliContext({ codemarieDir: options.codemarieDir })
	}
	 ...
	 */
	emitterForSession(sessionId: string): CodemarieSessionEmitter {
		let emitter = this.sessionEmitters.get(sessionId)
		if (!emitter) {
			emitter = new CodemarieSessionEmitter()
	...
	async initialize(params: acp.InitializeRequest, connection?: acp.AgentSideConnection): Promise<acp.InitializeResponse> {
		...
		await CodemarieEndpoint.initialize(this.ctx.EXTENSION_DIR)
		...
		return {
			...
			authMethods: [
				{
					id: "codemarie-oauth",
					name: "Sign in with Codemarie",
					description: "Authenticate with your Codemarie account via browser OAuth",
				},
				{
	...
	async newSession(params: acp.NewSessionRequest): Promise<acp.NewSessionResponse> {
		...
		Logger.debug("[CodemarieAgent] newSession called:", {
			sessionId,
			cwd: params.cwd,
		...
		const session: CodemarieAcpSession = {
			sessionId,
			cwd: params.cwd,
		...
		await this.sendAvailableCommands(sessionId, controller).catch((error) => {
			Logger.debug("[CodemarieAgent] Failed to send available commands:", error)
		})
	...
	private async getSessionModelState(mode: Mode): Promise<acp.SessionModelState> {
		...
		// Use provider-specific model ID key (e.g., codemarie uses actModeOpenRouterModelId)
	...
	async unstable_setSessionModel(params: acp.SetSessionModelRequest): Promise<acp.SetSessionModelResponse> {
		...
		Logger.debug("[CodemarieAgent] unstable_setSessionModel called:", {
		...
		// Update model ID using provider-specific keys (e.g., codemarie uses actModeOpenRouterModelId)
	 ...
	 * 2. Set up internal codemarie state subsription
	 * 3. Initialize or continue codemarie task
	 * 4. Translate CodemarieMessages to ACP SessionUpdates
	 * 5. Handle permission requests for tools/commands
	 * 6. Return when codemarie task completes, is cancelled, or needs user input
	 */
	async prompt(params: acp.PromptRequest): Promise<acp.PromptResponse> {
		...
		Logger.debug("[CodemarieAgent] prompt called:", {
			sessionId: params.sessionId,
		...
		try {
			...
			if (isLoadedSession && !hasActiveTask) {
				...
				Logger.debug("[CodemarieAgent] Resuming loaded session:", params.sessionId)
				...
				Logger.debug("[CodemarieAgent] Continuing existing task:", controller.task.taskId)

				// Find the last ask message and respond to it
				const messages = controller.task.messageStateHandler.getCodemarieMessages()
				...
				if (lastAskMessage) {
					...
					Logger.debug("[CodemarieAgent] No pending ask found, starting new task")
					await controller.initTask(textContent, imageContent, fileResources)
				}
			} else {
				// Start new task
				Logger.debug("[CodemarieAgent] Starting new task")
				await controller.initTask(textContent, imageContent, fileResources)
			}
			...
			if (controller.task) {
				...
				cleanupFunctions.push(() => {
					controller.task?.messageStateHandler.off("codemarieMessagesChanged", onCodemarieMessagesChanged)
				})
			}
			...
			for (const cleanup of cleanupFunctions) {
				try {
					...
					Logger.debug("[CodemarieAgent] Error during cleanup:", error)
				}
			}
	...
	private async handleCodemarieMessagesChanged(
		sessionId: string,
		sessionState: AcpSessionState,
		change: CodemarieMessageChange,
		resolvePrompt: (response: acp.PromptResponse) => void,
		promptResolved: { value: boolean },
	): Promise<void> {
		Logger.debug("[CodemarieAgent] handleCodemarieMessagesChanged:", change)
		try {
			switch (change.type) {
				case "add":
	 ...
	 * 3. Responds to Codemarie's ask based on the decision
	 *
	 * @param sessionId - The session ID
	 * @param sessionState - The session state
	 * @param message - The Codemarie ask message
	 * @param permissionRequest - The permission request details from translateMessage
	 */
	private async handlePermissionRequest(
		sessionId: string,
		sessionState: AcpSessionState,
		message: CodemarieMessageType,
		permissionRequest: Omit<acp.RequestPermissionRequest, "sessionId">,
	): Promise<void> {
		const session = this.sessions.get(sessionId)

		if (!session) {
			Logger.debug("[CodemarieAgent] No session found for permission request")
			return
		}
		...
		if (!controller?.task) {
			Logger.debug("[CodemarieAgent] No active task for permission request")
			return
		}

		const askType = message.ask as CodemarieAsk

		try {
			// Request permission from the client
			const response = await this.requestPermission(sessionId, permissionRequest.toolCall, permissionRequest.options)

			Logger.debug("[CodemarieAgent] Permission response received:", response.outcome)
	 ...
	 *
	 * This method uses translateMessage to properly map CodemarieMessages to ACP SessionUpdates,
	...
	private async processMessageWithDelta(
		...
		if (isTextStreamingMessage && message.text) {
			...
			if (result.requiresPermission && result.permissionRequest && !message.partial) {
				...
				// This will request permission from the client and respond to Codemarie
	...
	async cancel(params: acp.CancelNotification): Promise<void> {
		const session = this.sessions.get(params.sessionId)
		if (!session) {
			Logger.debug("[CodemarieAgent] cancel called for non-existent session:", params.sessionId)
			return
		}
		const sessionState = this.sessionStates.get(params.sessionId)

		Logger.debug("[CodemarieAgent] cancel called:", {
			sessionId: params.sessionId,
		...
		if (sessionState) {
			...
			if (controller?.task) {
				try {
					...
					Logger.debug("[CodemarieAgent] Error cancelling task:", error)
				}
			}
		}
	}
	...
	async setSessionMode(params: acp.SetSessionModeRequest): Promise<acp.SetSessionModeResponse> {
		...
		Logger.debug("[CodemarieAgent] setSessionMode called:", {
			sessionId: params.sessionId,
	 ...
	 * - Codemarie account authentication (codemarie-oauth)
	...
	async authenticate(params: acp.AuthenticateRequest): Promise<acp.AuthenticateResponse> {
		Logger.debug("[CodemarieAgent] authenticate called:", { methodId: params.methodId })
		...
		if (params.methodId !== "codemarie-oauth") {
		...
		Logger.debug("[CodemarieAgent] AuthHandler enabled, getting callback URL...")

		// Get the callback URL first to ensure the server is ready
		let callbackUrl: string
		try {
			callbackUrl = await authHandler.getCallbackUrl("/auth")
			Logger.debug("[CodemarieAgent] Callback URL ready:", callbackUrl)
		} catch (error) {
			Logger.error("[CodemarieAgent] Failed to get callback URL:", error)
		...
		try {
			...
			Logger.debug("[CodemarieAgent] Starting OAuth flow...")

			// Start the OAuth flow - this opens the browser
			await authService.createAuthRequest()

			Logger.debug("[CodemarieAgent] Browser opened, waiting for callback...")
			...
			while (Date.now() - startTime < AUTH_TIMEOUT_MS) {
				...
				const authData = stateManager.getSecretKey("codemarie:codemarieAccountId")
				if (authData) {
			...
			Logger.error("[CodemarieAgent] Authentication error:", error)
			throw error
		}
	}
	...
	private async emitSessionUpdate(sessionId: string, update: acp.SessionUpdate): Promise<void> {
		...
		try {
			...
			Logger.debug("[CodemarieAgent] Error emitting session update:", error)
	...
	protected async requestPermission(
		...
		Logger.debug("[CodemarieAgent] Requesting permission:", {
		...
		if (!this.permissionHandler) {
			...
			Logger.debug("[CodemarieAgent] No permission handler set, auto-rejecting")
	 ...
	 *
	 * This fetches commands from Codemarie's slash command system, filters out
	...
	private async sendAvailableCommands(sessionId: string, controller: Controller): Promise<void> {
		try {
			...
			Logger.debug("[CodemarieAgent] Sent available commands:", {
				sessionId,
				commandCount: availableCommands.length,
				commands: availableCommands.map((c) => c.name),
			})
		} catch (error) {
			Logger.debug("[CodemarieAgent] Error sending available commands:", error)
		}
	}
	...
	private async authenticateOpenAiCodex(): Promise<acp.AuthenticateResponse> {
		Logger.debug("[CodemarieAgent] Starting OpenAI Codex OAuth flow...")

		try {
			// Get the authorization URL and start the callback server
			const authUrl = openAiCodexOAuthManager.startAuthorizationFlow()

			Logger.debug("[CodemarieAgent] Opening browser for OpenAI Codex auth:", authUrl)
			...
			Logger.debug("[CodemarieAgent] OpenAI Codex authentication successful")
			...
			Logger.error("[CodemarieAgent] OpenAI Codex authentication error:", error)
			throw error
		}
	}
}
