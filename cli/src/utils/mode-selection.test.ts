...
describe("selectOutputMode", () => {
	...
	describe("piped stdin", () => {
		it("should use plain text mode when stdin was piped (echo x | codemarie)", () => {
			const result = selectOutputMode({
				stdoutIsTTY: true,
		...
		it("should use plain text mode when stdin was piped but empty (echo '' | codemarie 'prompt')", () => {
			const result = selectOutputMode({
				stdoutIsTTY: true,
				stdinIsTTY: false,
	...
	describe("stdout redirected", () => {
		...
		it("should use plain text mode when stdout is piped", () => {
			// codemarie "prompt" | grep something
			const result = selectOutputMode({
				stdoutIsTTY: false,
				stdinIsTTY: true,
				stdinWasPiped: false,
	...
	describe("real-world scenarios", () => {
		it("codemarie (no args, interactive terminal)", () => {
			const result = selectOutputMode({
				stdoutIsTTY: true,
				stdinIsTTY: true,
				stdinWasPiped: false,
		...
		it('codemarie "prompt" (prompt arg, interactive terminal)', () => {
			const result = selectOutputMode({
				stdoutIsTTY: true,
				stdinIsTTY: true,
				stdinWasPiped: false,
		...
		it('codemarie "prompt" > output.log', () => {
			const result = selectOutputMode({
				stdoutIsTTY: false,
				stdinIsTTY: true,
				stdinWasPiped: false,
