...
export function validateModelId(
	...
	codemarieModels?: Record<string, ModelInfo>,
): string | undefined {
	if (apiConfiguration) {
		const { apiProvider, openRouterModelId, codemarieModelId } = getModeSpecificFields(apiConfiguration, currentMode)
		switch (apiProvider) {
			case "openrouter":
				...
				break
			case "codemarie":
				const codemarieResolvedModelId = codemarieModelId || openRouterDefaultModelId
				if (!codemarieResolvedModelId) {
					return "You must provide a model ID."
				}
				if (codemarieModels && !Object.keys(codemarieModels).includes(codemarieResolvedModelId)) {
					return "The model ID you provided is not available. Please choose a different model."
				}
				break
		}
