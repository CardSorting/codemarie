...
export interface CodemarieModelPickerProps {
	isPopup?: boolean
	currentMode: Mode
...
function toFeaturedModelCardEntry(
	model: Pick<CodemarieRecommendedModel, "id" | "description" | "tags">,
...
const CodemarieModelPicker: React.FC<CodemarieModelPickerProps> = ({ isPopup, currentMode, showProviderRouting, initialTab }) => {
	const { handleModeFieldsChange, handleFieldChange } = useApiConfigurationHandlers()
	const { apiConfiguration, favoritedModelIds, codemarieModels, refreshCodemarieModels } = useExtensionState()
	const modeFields = getModeSpecificFields(apiConfiguration, currentMode)
	const [searchTerm, setSearchTerm] = useState(modeFields.codemarieModelId || openRouterDefaultModelId)
	const [isDropdownVisible, setIsDropdownVisible] = useState(false)
	const [selectedIndex, setSelectedIndex] = useState(-1)
	const [codemarieRecommendedModels, setCodemarieRecommendedModels] = useState<FeaturedModelCardEntry[]>([])
	...
	const freeCodemarieModelIdSet = useMemo(
		() => new Set(freeCodemarieModelIds.map((modelId) => normalizeModelId(modelId))),
		[freeCodemarieModelIds],
	)
	...
	const refreshCodemarieRecommendedModels = useCallback(async (): Promise<boolean> => {
		try {
			const response = await ModelsServiceClient.makeUnaryRequest(
				...
				EmptyRequest.create({}),
				EmptyRequest.toJSON,
				CodemarieRecommendedModelsResponse.fromJSON,
			)
			...
			const free = (response.free ?? [])
				.map((model) => toFeaturedModelCardEntry(model, "FREE"))
				.filter((model): model is FeaturedModelCardEntry => model !== null)
			setCodemarieRecommendedModels(recommended)
	...
	const clearCodemarieRecommendedModelsRetryTimeout = useCallback(() => {
		if (codemarieRecommendedModelsRetryTimeoutRef.current !== null) {
			window.clearTimeout(codemarieRecommendedModelsRetryTimeoutRef.current)
			codemarieRecommendedModelsRetryTimeoutRef.current = null
		}
	}, [])

	const fetchCodemarieRecommendedModels = useCallback(async () => {
		if (hasSuccessfulCodemarieRecommendedModelsFetchRef.current || isFetchingCodemarieRecommendedModelsRef.current) {
			return
		}
		isFetchingCodemarieRecommendedModelsRef.current = true
		const succeeded = await refreshCodemarieRecommendedModels()
		isFetchingCodemarieRecommendedModelsRef.current = false

		if (succeeded) {
			hasSuccessfulCodemarieRecommendedModelsFetchRef.current = true
			clearCodemarieRecommendedModelsRetryTimeout()
			return
		}

		if (codemarieRecommendedModelsRetryTimeoutRef.current === null) {
			codemarieRecommendedModelsRetryTimeoutRef.current = window.setTimeout(() => {
				codemarieRecommendedModelsRetryTimeoutRef.current = null
				void fetchCodemarieRecommendedModels()
			}, CLINE_RECOMMENDED_MODELS_RETRY_DELAY_MS)
		}
	}, [clearCodemarieRecommendedModelsRetryTimeout, refreshCodemarieRecommendedModels])
	...
	useEffect(() => {
		...
		const currentModelId = modeFields.codemarieModelId || openRouterDefaultModelId
		setActiveTab(freeCodemarieModelIdSet.has(normalizeModelId(currentModelId)) ? "free" : "recommended")
	}, [modeFields.codemarieModelId, freeCodemarieModelIdSet, initialTab])
	...
	const handleModelChange = (newModelId: string) => {
		...
		handleModeFieldsChange(
			{
				codemarieModelId: { plan: "planModeCodemarieModelId", act: "actModeCodemarieModelId" },
				codemarieModelInfo: { plan: "planModeCodemarieModelInfo", act: "actModeCodemarieModelInfo" },
			},
			{
				codemarieModelId: newModelId,
				codemarieModelInfo: codemarieModels?.[newModelId],
			},
			currentMode,
		)
	}
	...
	const { selectedModelId, selectedModelInfo } = useMemo(() => {
		...
		if (freeCodemarieModelIdSet.has(normalizeModelId(selected.selectedModelId))) {
			return {
				...selected,
				selectedModelInfo: {
		...
		return selected
	}, [apiConfiguration, currentMode, freeCodemarieModelIdSet])
	...
	useEffect(() => {
		void fetchCodemarieRecommendedModels()
	}, [fetchCodemarieRecommendedModels])

	// Sync external changes when the modelId changes
	useEffect(() => {
		const currentModelId = modeFields.codemarieModelId || openRouterDefaultModelId
		setSearchTerm(currentModelId)
	}, [modeFields.codemarieModelId])
	...
	const modelIds = useMemo(() => {
		const unfilteredModelIds = Object.keys(codemarieModels ?? {}).sort((a, b) => a.localeCompare(b))
		return filterOpenRouterModelIds(unfilteredModelIds, "codemarie", freeCodemarieModelIds)
	}, [codemarieModels, freeCodemarieModelIds])
	...
	const showBudgetSlider = useMemo(() => {
		...
		return (
			Object.entries(codemarieModels ?? {})?.some(([id, m]) => id === selectedModelId && m.thinkingConfig) ||
	...
	}, [codemarieModels, selectedModelId, selectedModelIdLower, showReasoningEffort])
...
export default CodemarieModelPicker
