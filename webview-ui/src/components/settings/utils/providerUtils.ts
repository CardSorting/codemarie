	...
	switch (provider) {
		...
		case "codemarie":
			...
			const codemarieModelId =
				(currentMode === "plan" ? apiConfiguration?.planModeCodemarieModelId : apiConfiguration?.actModeCodemarieModelId) ||
				fallbackOpenRouterModelId ||
				openRouterDefaultModelId
			const codemarieModelInfo =
				(currentMode === "plan" ? apiConfiguration?.planModeCodemarieModelInfo : apiConfiguration?.actModeCodemarieModelInfo) ||
...
export function getModeSpecificFields(apiConfiguration: ApiConfiguration | undefined, mode: Mode) {
	...
	const codemarieModelId =
		(mode === "plan" ? apiConfiguration.planModeCodemarieModelId : apiConfiguration.actModeCodemarieModelId) || openRouterModelId
	const codemarieModelInfo =
