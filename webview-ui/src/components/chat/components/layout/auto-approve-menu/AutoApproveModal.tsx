...
const AutoApproveModal: React.FC<AutoApproveModalProps> = ({ isVisible, setIsVisible, buttonRef, ACTION_METADATA }) => {
	const { autoApprovalSettings } = useGlobalState()
	...
	return (
		<div ref={modalRef}>
			...
			<div
				...
				<button
					className="mb-2.5 text-muted-foreground text-xs cursor-pointer bg-transparent border-none p-0 text-left block w-full"
					onClick={() => setIsVisible(false)}
					type="button">
					Let Codemarie take these actions without asking for approval.{" "}
					<a
						className="text-link hover:text-link-hover"
						href="https://docs.codemarie.bot/features/auto-approve#auto-approve"
						rel="noopener"
