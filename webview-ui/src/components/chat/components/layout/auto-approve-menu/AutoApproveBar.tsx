...
const AutoApproveBar = ({ style }: AutoApproveBarProps) => {
	const { autoApprovalSettings, yoloModeToggled } = useGlobalState()
	const { navigateToSettings } = useNavigation()

	const [isModalVisible, setIsModalVisible] = useState(false)
	const buttonRef = useRef<HTMLButtonElement>(null)

	const handleNavigateToFeatures = (e: React.MouseEvent | React.KeyboardEvent) => {
		e.preventDefault()
		e.stopPropagation()
	...
	return (
		<div
			...
			<button
				...
				className="group cursor-pointer pt-3 pb-3.5 pr-2 px-3.5 flex items-center justify-between gap-0 bg-transparent border-none w-full text-inherit"
				onClick={() => {
