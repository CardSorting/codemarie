...
async function verifyCollision() {
    ...
    // 0. Setup streams
    await dbPool.push({ type: "insert", table: "agent_streams", values: { id: agentA, status: "active", createdAt: Date.now() }, layer: "infrastructure" });
    await dbPool.push({ type: "insert", table: "agent_streams", values: { id: agentB, status: "active", createdAt: Date.now() }, layer: "infrastructure" });
    await dbPool.flush();
