...
This directory contains the end-to-end tests for the CodeMarie VS Code extension using Playwright. These tests simulate user interactions with the extension in a real VS Code environment.
...
- **`fixtures/server/`** - Mock API server for testing CodeMarie's backend interactions
...
- **`sidebar`** - Playwright Frame object for the CodeMarie extension's sidebar
...
2. **Playwright will open:**
   - A VS Code window with CodeMarie extension loaded
...
- **VS Code Configuration:**
  ...
  - Extension development mode with CodeMarie loaded
  - Temporary user data and extensions directories

- **Mock API Server:**
  - Runs on `http://localhost:7777`
  - Provides mock responses for CodeMarie API calls
