 ...
 * Designed to be instantiated once in Codemarie.ts and passed to file manipulation services.
 * Uses the 'ignore' library to support standard .gitignore syntax in .codemarieignore files.
 */
export class CodemarieIgnoreController {
	private cwd: string
	private ignoreInstance: Ignore
	private fileWatcher?: FSWatcher
	codemarieIgnoreContent: string | undefined

	constructor(cwd: string) {
		this.cwd = cwd
		this.ignoreInstance = ignore()
		this.codemarieIgnoreContent = undefined
	}
	...
	async initialize(): Promise<void> {
		// Set up file watcher for .codemarieignore
		this.setupFileWatcher()
		await this.loadCodemarieIgnore()
	}

	/**
	 * Set up the file watcher for .codemarieignore changes
	 */
	private setupFileWatcher(): void {
		const ignorePath = path.join(this.cwd, ".codemarieignore")
		...
		this.fileWatcher.on("error", (error) => {
			Logger.error("Error watching .codemarieignore file:", error)
		})
	}

	/**
	 * Load custom patterns from .codemarieignore if it exists.
	 * Supports "!include <filename>" to load additional ignore patterns from other files.
	 */
	private async loadCodemarieIgnore(): Promise<void> {
		try {
			...
			if (await fileExistsAtPath(ignorePath)) {
				...
				this.codemarieIgnoreContent = content
				await this.processIgnoreContent(content)
				this.ignoreInstance.add(".codemarieignore")
			} else {
				this.codemarieIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			Logger.error("Unexpected error loading .codemarieignore:", error)
		}
	}
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processCodemarieIgnoreIncludes(content)
	 ...
	 */
	private async processCodemarieIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			Logger.debug(`[CodemarieIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .codemarieignore does not exist
		if (!this.codemarieIgnoreContent) {
			return true
		}
		try {
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .codemarieignore exists
		if (!this.codemarieIgnoreContent) {
			return undefined
		}
