...
import { CodemarieIgnoreController } from "./CodemarieIgnoreController"
import "should"

describe("CodemarieIgnoreController", () => {
	let tempDir: string
	let controller: CodemarieIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .codemarieignore file
		await fs.writeFile(
			path.join(tempDir, ".codemarieignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new CodemarieIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .codemarieignore file", async () => {
			const result = controller.validateAccess(".codemarieignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .codemarieignore", async () => {
			// Create a new .codemarieignore with comments
			await fs.writeFile(
				path.join(tempDir, ".codemarieignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .codemarieignore gracefully", async () => {
			// Create a new controller in a directory without .codemarieignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new CodemarieIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .codemarieignore", async () => {
			await fs.writeFile(path.join(tempDir, ".codemarieignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .codemarieignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".codemarieignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .codemarieignore
			controller = new CodemarieIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .codemarieignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .codemarieignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".codemarieignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".codemarieignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
