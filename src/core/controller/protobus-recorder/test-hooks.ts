import { Controller } from "@/core/controller"
import { ProtobusRecorderBuilder } from "@/core/controller/protobus-recorder/protobus-recorder.builder"
import { ProtobusPostRecordHook } from "@/core/controller/protobus-recorder/types"
import { getLatestState } from "@/core/controller/state/getLatestState"

// Add 50ms delay by default to ensure we get the latest state
const TEST_HOOK_LATEST_STATE_DELAY = 50

export function testHooks(controller: Controller): ProtobusPostRecordHook[] {
	return [
		async (entry) => {
			ProtobusRecorderBuilder.getRecorder(controller).cleanupSyntheticEntries()

			await new Promise((resolve) => setTimeout(resolve, TEST_HOOK_LATEST_STATE_DELAY))

			const requestId = entry.requestId

			// Record synthetic "getLatestState" request
			ProtobusRecorderBuilder.getRecorder(controller).recordRequest(
				{
					service: "codemarie.StateService",
					method: "getLatestState",
					message: {},
					request_id: requestId,
					is_streaming: false,
				},
				true,
			)

			const state = await getLatestState(controller, {})

			ProtobusRecorderBuilder.getRecorder(controller).recordResponse(requestId, {
				request_id: requestId,
				message: state,
			})
		},
	]
}
