...
import { UpdateApiConfigurationRequestNew as UpdateApiConfigurationRequest } from "@shared/proto/codemarie/system"
...
export async function updateApiConfigurationProto(
	...
	try {
		...
		// Merge the updates provided in the request
		const protoApiConfiguration = {
			// biome-ignore lint/suspicious/noExplicitAny: options can be any
			...(request.updates?.options as any),
			// biome-ignore lint/suspicious/noExplicitAny: secrets can be any
			...(request.updates?.secrets as any),
		}
		...
		const convertedApiConfigurationFromProto = {
			...
			planModeApiProvider:
				protoApiConfiguration.planModeApiProvider !== undefined
					? // biome-ignore lint/style/noNonNullAssertion: planModeApiProvider is checked above
						convertProtoToApiProvider(protoApiConfiguration.planModeApiProvider!)
					: undefined,
			actModeApiProvider:
				protoApiConfiguration.actModeApiProvider !== undefined
					? // biome-ignore lint/style/noNonNullAssertion: actModeApiProvider is checked above
						convertProtoToApiProvider(protoApiConfiguration.actModeApiProvider!)
					: undefined,
			embeddingProvider:
				protoApiConfiguration.embeddingProvider !== undefined
					? // biome-ignore lint/style/noNonNullAssertion: embeddingProvider is checked above
