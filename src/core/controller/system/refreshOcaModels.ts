import { ApiFormat, StringRequest } from "@shared/proto/codemarie/common"
import { OcaCompatibleModelInfo, OcaModelInfo } from "@shared/proto/codemarie/system"
...
export async function refreshOcaModels(controller: Controller, request: StringRequest): Promise<OcaCompatibleModelInfo> {
	...
	// biome-ignore lint/style/noNonNullAssertion: ocaAccessToken is checked elsewhere
	...
	try {
		...
		if (response.data?.data) {
			...
			const planModeSelectedModelId =
				apiConfiguration?.planModeOcaModelId && models[apiConfiguration.planModeOcaModelId]
					...
					: // biome-ignore lint/style/noNonNullAssertion: defaultModelId is set if models are found
						defaultModelId!
			...
			// biome-ignore lint/suspicious/noImplicitAnyLet: reasoning effort can be any
		...
		// biome-ignore lint/suspicious/noImplicitAnyLet: userMsg is set in the block
		let userMsg
		if (err.response) {
...
// biome-ignore lint/suspicious/noExplicitAny: modelSupportedApiList from API can be any
