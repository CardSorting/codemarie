...
import { UpdateApiConfigurationPartialRequest } from "@shared/proto/codemarie/system"
...
export async function updateApiConfigurationPartial(
	...
	try {
		...
		// biome-ignore lint/suspicious/noExplicitAny: request.apiConfiguration is any
		const newConfigValues = convertProtoToApiConfiguration(request.apiConfiguration as any) as any

		// Apply only the fields specified in the mask
		const updatedConfig = { ...currentConfig }
		for (const field of request.updateMask) {
			// biome-ignore lint/suspicious/noExplicitAny: updatedConfig is a partial config
