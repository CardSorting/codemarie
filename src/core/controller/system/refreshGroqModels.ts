		...
		if (!groqApiKey) {
			...
			for (const [modelId, modelInfo] of Object.entries(groqModels)) {
				models[modelId] = {
					...
					// biome-ignore lint/suspicious/noExplicitAny: price fields might not be in the base model interface
			...
			if (response.data?.data) {
				...
				for (const rawModel of rawModels) {
					...
					const modelInfo: Partial<ModelInfo> = {
						...
						// biome-ignore lint/suspicious/noExplicitAny: static model info can be any
		...
		if (cachedModels && Object.keys(cachedModels).length > 0) {
			...
			for (const [modelId, modelInfo] of Object.entries(groqModels)) {
				models[modelId] = {
					...
					inputPrice: modelInfo.inputPrice,
					outputPrice: modelInfo.outputPrice,
					// biome-ignore lint/suspicious/noExplicitAny: model info from static map can be any
					cacheWritesPrice: (modelInfo as any).cacheWritesPrice || 0,
					// biome-ignore lint/suspicious/noExplicitAny: model info from static map can be any
					cacheReadsPrice: (modelInfo as any).cacheReadsPrice || 0,
					description: modelInfo.description || `${modelId} model`,
 ...
 */
// biome-ignore lint/suspicious/noExplicitAny: rawModel from API is any
 ...
 */
// biome-ignore lint/suspicious/noExplicitAny: rawModel and staticModelInfo can be any
