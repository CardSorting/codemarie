...
/**
 * The raw model information returned by the Codemarie API to list models
 */
interface CodemarieRawModelInfo {
	id: string
	name: string
	...
	// biome-ignore lint/suspicious/noExplicitAny: tiers from API can have various structures
	tiers?: any[] | null
	supported_parameters?: CodemarieSupportedParams[] | null
}
...
async function fetchRawCodemarieModels(): Promise<CodemarieRawModelInfo[]> {
	const apiBaseUrl = CodemarieEnv.config().apiBaseUrl
	const response = await axios.get(`${apiBaseUrl}/api/v1/ai/codemarie/models`, getAxiosSettings())

	if (!Array.isArray(response.data?.data)) {
		throw new Error("Invalid response data when fetching Codemarie models")
	}

	Logger.log("Codemarie models source: Codemarie API")
	return response.data.data as CodemarieRawModelInfo[]
}

/**
 * Core function: Refreshes the Codemarie models and returns application types
...
export async function refreshCodemarieModels(controller: Controller): Promise<Record<string, ModelInfo>> {
	...
	const cache = StateManager.get().getModelsCache("codemarie")
	if (cache) {
		return cache
	}
	...
	pendingRefresh = (async () => {
		try {
			return await fetchAndCacheCodemarieModels()
		} finally {
...
async function fetchAndCacheCodemarieModels(): Promise<Record<string, ModelInfo>> {
	const codemarieModelsFilePath = path.join(await ensureCacheDirectoryExists(), GlobalFileNames.codemarieModels)

	let models: Record<string, ModelInfo> = {}
	try {
		const rawModels = await fetchRawCodemarieModels()
		// biome-ignore lint/suspicious/noExplicitAny: price from API can be any
		...
		if (Object.keys(models).length === 0) {
			throw new Error("No Codemarie models returned from API")
		}
		// Save models and cache them in memory
		await fs.writeFile(codemarieModelsFilePath, JSON.stringify(models))
		Logger.log("Codemarie models fetched and saved")
	} catch (error) {
		Logger.error("Error fetching Codemarie models:", error)

		// If we failed to fetch models, try to read cached models from disk
		try {
			const fileExists = await fileExistsAtPath(codemarieModelsFilePath)
			if (fileExists) {
				const fileContents = await fs.readFile(codemarieModelsFilePath, "utf8")
				models = JSON.parse(fileContents)
				Logger.log("Loaded Codemarie models from cache")
			}
		} catch (cacheError) {
			Logger.error("Error reading Codemarie models from cache:", cacheError)
		}
	}

	// Avoid poisoning in-memory cache with an empty model map after transient failures.
	if (Object.keys(models).length > 0) {
		StateManager.get().setModelsCache("codemarie", models)
	}
...
/**
 * Read cached Codemarie models from disk
 * @returns The cached models or undefined if not found
 */
export async function readCodemarieModelsFromCache(): Promise<Record<string, ModelInfo> | undefined> {
	try {
		const codemarieModelsFilePath = path.join(await ensureCacheDirectoryExists(), GlobalFileNames.codemarieModels)
		const fileExists = await fileExistsAtPath(codemarieModelsFilePath)
		if (fileExists) {
		...
		Logger.error("Error reading Codemarie models from cache:", error)
	}
	return undefined
}
