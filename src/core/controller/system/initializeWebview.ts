...
import { sendOpenRouterModelsEvent } from "./subscribeToOpenRouterModels"
...
export async function initializeWebview(controller: Controller, _request: EmptyRequest): Promise<Empty> {
	try {
		...
		refreshOpenRouterModels(controller).then(async (models: any) => {
		...
		// biome-ignore lint/suspicious/noExplicitAny: models from API can be any
		refreshGroqModels(controller).then(async (models: any) => {
			if (models && Object.keys(models).length > 0) {
		...
		// biome-ignore lint/suspicious/noExplicitAny: models from API can be any
		refreshBasetenModels(controller).then(async (models: any) => {
			if (models && Object.keys(models).length > 0) {
		...
		// biome-ignore lint/suspicious/noExplicitAny: response from API can be any
		refreshHicapModels(controller, EmptyRequest.create()).then(async (response: any) => {
			if (response?.models) {
				// Update model info in state (this needs to be done here since we don't want to update state while settings is open, and we may refresh models there)
				const apiConfiguration = controller.stateManager.getApiConfiguration()
				const planActSeparateModelsSetting = controller.stateManager.getGlobalSettingsKey("planActSeparateModelsSetting")
