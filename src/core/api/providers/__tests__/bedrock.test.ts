...
describe("AwsBedrockHandler", () => {
	...
	describe("tool config mapping", () => {
		it("should map Anthropic tools to Bedrock toolConfig", () => {
			...
			const toolConfig = handler.mapCodemarieToolsToBedrockToolConfig([
				{
					name: "read_file",
		...
		it("should return undefined when tools is undefined or empty", () => {
			const handler = new AwsBedrockHandler(mockOptions)
			should.not.exist(handler.mapCodemarieToolsToBedrockToolConfig(undefined))
			should.not.exist(handler.mapCodemarieToolsToBedrockToolConfig([]))
		})
	...
	describe("native tool calling integration", () => {
		...
		it("should format a complete tool call round-trip correctly", () => {
			// Simulates the full cycle: model returns tool_use → Codemarie executes → sends tool_result back
			const handler = new AwsBedrockHandler(mockOptions)
