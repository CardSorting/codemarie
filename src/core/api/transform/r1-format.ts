...
import { CodemarieAssistantThinkingBlock, CodemarieStorageMessage } from "@/shared/messages/content"
...
export function addReasoningContent(
	...
	originalMessages: CodemarieStorageMessage[],
		...
		if (msg.role === "assistant") {
			if (Array.isArray(msg.content)) {
				const thinking = msg.content
					.filter((p): p is CodemarieAssistantThinkingBlock => p.type === "thinking")
					.map((p) => p.thinking)
					.join("\n")
				if (thinking) {
