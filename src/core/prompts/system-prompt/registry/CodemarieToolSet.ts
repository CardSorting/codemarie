...
import { type CodemarieToolSpec, toolSpecFunctionDeclarations, toolSpecFunctionDefinition, toolSpecInputSchema } from "../spec"
import { PromptVariant, SystemPromptContext } from "../types"

export class CodemarieToolSet {
	// A list of tools mapped by model group
	private static variants: Map<ModelFamily, Set<CodemarieToolSet>> = new Map()

	private constructor(
		public readonly id: string,
		public readonly config: CodemarieToolSpec,
	) {
		this._register()
	}

	public static register(config: CodemarieToolSpec): CodemarieToolSet {
		return new CodemarieToolSet(config.id, config)
	}

	private _register(): void {
		const existingTools = CodemarieToolSet.variants.get(this.config.variant) || new Set()
		if (!Array.from(existingTools).some((t) => t.config.id === this.config.id)) {
			existingTools.add(this)
			CodemarieToolSet.variants.set(this.config.variant, existingTools)
		}
	}

	public static getTools(variant: ModelFamily): CodemarieToolSet[] {
	...
	public static getRegisteredModelIds(): string[] {
		return Array.from(CodemarieToolSet.variants.keys())
	}

	public static getToolByName(toolName: string, variant: ModelFamily): CodemarieToolSet | undefined {
		const tools = CodemarieToolSet.getTools(variant)
	...
	public static getToolByNameWithFallback(toolName: string, variant: ModelFamily): CodemarieToolSet | undefined {
		// Try exact variant first
		const exact = CodemarieToolSet.getToolByName(toolName, variant)
		if (exact) {
			return exact
		}

		// Fallback to GENERIC
		const generic = CodemarieToolSet.getToolByName(toolName, ModelFamily.GENERIC)
		if (generic) {
			return generic
		}

		// Final fallback: search across all registered variants
		for (const [, tools] of CodemarieToolSet.variants) {
	...
	public static getToolsForVariantWithFallback(variant: ModelFamily, requestedIds: string[]): CodemarieToolSet[] {
		const resolved: CodemarieToolSet[] = []
		for (const id of requestedIds) {
			const tool = CodemarieToolSet.getToolByNameWithFallback(id, variant)
			if (tool) {
	...
	public static getEnabledTools(variant: PromptVariant, context: SystemPromptContext): CodemarieToolSet[] {
		const resolved: CodemarieToolSet[] = []
		const requestedIds = variant.tools ? [...variant.tools] : []
		for (const id of requestedIds) {
			const tool = CodemarieToolSet.getToolByNameWithFallback(id, variant.family)
			if (tool) {
				// Avoid duplicates by id
				if (!resolved.some((t) => t.config.id === tool.config.id)) {
					resolved.push(tool)
	...
	private static getDynamicSubagentToolSpecs(variant: PromptVariant, context: SystemPromptContext): CodemarieToolSpec[] {
		if (context.subagentsEnabled !== true || context.isSubagentRun) {
			return []
		}

		const requestedIds = variant.tools ? [...variant.tools] : []
		const shouldIncludeSubagentTools = requestedIds.length === 0 || requestedIds.includes(CodemarieDefaultTool.USE_SUBAGENTS)
		...
		return agentConfigs.map(({ toolName, config }) => ({
			...
			id: CodemarieDefaultTool.USE_SUBAGENTS,
			name: toolName,
	...
	public static getEnabledToolSpecs(variant: PromptVariant, context: SystemPromptContext): CodemarieToolSpec[] {
		const registeredTools = CodemarieToolSet.getEnabledTools(variant, context).map((tool) => tool.config)
		const dynamicSubagentTools = CodemarieToolSet.getDynamicSubagentToolSpecs(variant, context)

		const includesDynamicSubagents = dynamicSubagentTools.length > 0
		const filteredRegistered = includesDynamicSubagents
			? registeredTools.filter((tool) => tool.id !== CodemarieDefaultTool.USE_SUBAGENTS)
			: registeredTools
	...
	public static getNativeTools(variant: PromptVariant, context: SystemPromptContext) {
		...
		// Base set
		const toolConfigs = CodemarieToolSet.getEnabledToolSpecs(variant, context)

		// MCP tools
		const mcpServers = context.mcpHub?.getServers()?.filter((s) => s.disabled !== true) || []
		const mcpTools = mcpServers?.flatMap((server) => mcpToolToCodemarieToolSpec(variant.family, server))
...
/**
 * Convert an MCP server's tools to CodemarieToolSpec format
 */
export function mcpToolToCodemarieToolSpec(family: ModelFamily, server: McpServer): CodemarieToolSpec[] {
