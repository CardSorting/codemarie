...
import { CodemarieApiReqCancelReason, CodemarieApiReqInfo } from "@/shared/ExtensionMessage"
...
export const updateApiReqMsg = async (params: UpdateApiReqMsgParams) => {
	const codemarieMessages = params.messageStateHandler.getCodemarieMessages()
	const currentApiReqInfo: CodemarieApiReqInfo = JSON.parse(codemarieMessages[params.lastApiReqIndex].text || "{}")
	delete currentApiReqInfo.retryStatus // Clear retry status when request is finalized

	await params.messageStateHandler.updateCodemarieMessage(params.lastApiReqIndex, {
		text: JSON.stringify({
