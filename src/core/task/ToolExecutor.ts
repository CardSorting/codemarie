	...
	private async handleCompleteBlock(block: ToolUse, config: any): Promise<void> {
		...
		try {
			...
			if (
				...
				if (postExecResult.warning) {
					// Violations found — surface warning with correction guidance
					const hint = postExecResult.correctionHint ? `\n${postExecResult.correctionHint}` : ""
					this.say("text", `📍 Written to **${layerLabel}** layer\n${postExecResult.warning}${hint}`).catch(() => {})
				} else {
					// Clean write — brief layer confirmation
					this.say("text", `✅ Written to **${layerLabel}** layer — no architectural issues`).catch(() => {})
				}
