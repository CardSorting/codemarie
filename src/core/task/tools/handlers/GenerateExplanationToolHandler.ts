...
import type { CodemarieSayGenerateExplanation } from "@/shared/ExtensionMessage"
...
/**
 * Helper to create a stringified CodemarieSayGenerateExplanation message
 */
function createExplanationMessage(
	title: string,
	fromRef: string,
	toRef: string,
	status: CodemarieSayGenerateExplanation["status"],
	error?: string,
): string {
	const message: CodemarieSayGenerateExplanation = { title, fromRef, toRef, status }
	if (error) {
		message.error = error
	}
...
export class GenerateExplanationToolHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = CodemarieDefaultTool.GENERATE_EXPLANATION
