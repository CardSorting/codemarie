...
export class Workspace {
	...
	async createRepo(repoId: string, defaultBranch = "main"): Promise<Repository> {
	...
	async listRepos(options: { limit?: number; startAfter?: string } = {}): Promise<string[]> {
		const rows = await this.db.selectWhere("repositories", [{ column: "workspaceId", value: this.workspaceId }])
