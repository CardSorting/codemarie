...
type MessageStateHandlerLike = {
	getCodemarieMessages(): CodemarieMessageLike[]
}
...
/**
 * Builds the evaluation context used for conditional Codemarie Rules (e.g. YAML frontmatter `paths:`).
 *
 * Kept in the user-instructions domain so Task remains orchestration-focused.
 *
 * Path context is gathered from multiple sources in codemarieMessages:
...
export class RuleContextBuilder {
	...
	private static async getRulePathContext(deps: RuleContextBuilderDeps): Promise<string[]> {
		...
		const codemarieMessages = deps.messageStateHandler.getCodemarieMessages()
		...
		// (3) Files edited by Codemarie during this task (completed operations):
