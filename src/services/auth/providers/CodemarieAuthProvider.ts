...
import { AuthInvalidTokenError, AuthNetworkError } from "@/services/error/CodemarieError"
...
interface CodemarieAuthApiUser {
	...
	email: string
	name: string
	codemarieUserId: string | null
	accounts: string[] | null
}

// Unified API response data shape for token exchange/refresh
interface CodemarieAuthResponseData {
	/**
...
export interface CodemarieAuthApiTokenExchangeResponse {
	success: boolean
	data: CodemarieAuthResponseData
}

export interface CodemarieAuthApiTokenRefreshResponse {
	success: boolean
	data: CodemarieAuthResponseData
}

export class CodemarieAuthProvider {
	readonly name = "codemarie"
	...
	private clearSession(controller: Controller, reason: string, storedAuthData?: CodemarieAuthInfo) {
		Logger.error(reason)
		...
		controller.stateManager.setSecret("codemarie:codemarieAccountId", undefined)
		this.refreshRetryCount = 0
		this.lastRefreshAttempt = 0
		return null
	}

	private logFailedRefreshAttempt(response: Response, storedAuthData?: CodemarieAuthInfo) {
		const startedAt = storedAuthData?.startedAt
		const timeSinceStarted = Date.now() - (startedAt || 0)
	...
	/**
	 * Retrieves Codemarie auth info using the stored access token.
	 * @param controller - The controller instance to access stored secrets.
	 * @returns {Promise<CodemarieAuthInfo | null>} A promise that resolves with the auth info or null.
	 */
	async retrieveCodemarieAuthInfo(controller: Controller): Promise<CodemarieAuthInfo | null> {
		try {
			// Get the stored auth data from secure storage
			const storedAuthDataString = controller.stateManager.getSecretKey("codemarie:codemarieAccountId")
			...
			let storedAuthData: CodemarieAuthInfo
			try {
			...
			if (await this.shouldRefreshIdToken(storedAuthData.refreshToken, storedAuthData.expiresAt)) {
				...
				try {
					...
					if (newAuthInfoString !== storedAuthDataString) {
						controller.stateManager.setSecret("codemarieAccountId", undefined) // cleanup old key
						controller.stateManager.setSecret("codemarie:codemarieAccountId", newAuthInfoString)
					}
					...
					// when the user actually tries to use Codemarie, not at startup
					return storedAuthData
				}
			}
	 ...
	 * @returns {Promise<CodemarieAuthInfo>} The new access token and user info.
	 */
	async refreshToken(refreshToken: string, storedData: CodemarieAuthInfo): Promise<CodemarieAuthInfo> {
		try {
			const endpoint = new URL(CODEMARIE_API_ENDPOINT.REFRESH_TOKEN, this.config.apiBaseUrl)
			...
			const data: CodemarieAuthApiTokenExchangeResponse = await response.json()
	...
	async getAuthRequest(callbackUrl: string): Promise<string> {
		const authUrl = new URL(CODEMARIE_API_ENDPOINT.AUTH, this.config.apiBaseUrl)
	...
	async signIn(controller: Controller, authorizationCode: string, provider: string): Promise<CodemarieAuthInfo | null> {
		try {
			...
			const tokenUrl = new URL(CODEMARIE_API_ENDPOINT.TOKEN_EXCHANGE, this.config.apiBaseUrl)
			...
			const responseType: CodemarieAuthApiTokenExchangeResponse = responseJSON
			...
			controller.stateManager.setSecret("codemarie:codemarieAccountId", JSON.stringify(codemarieAuthInfo))

			return codemarieAuthInfo
		} catch (error) {
	...
	private async fetchRemoteUserInfo(tokenData: CodemarieAuthApiTokenExchangeResponse["data"]): Promise<CodemarieAccountUserInfo> {
		try {
			...
			return {
				id: tokenData.userInfo.codemarieUserId || "",
