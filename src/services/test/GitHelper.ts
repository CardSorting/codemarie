...
export async function validateWorkspacePath(workspacePath: string): Promise<void> {
	...
	try {
		const testFile = path.join(workspacePath, ".codemarie_write_test")
...
export async function initializeGitRepository(workspacePath: string): Promise<boolean> {
	...
	try {
		...
		await execa("git", ["config", "user.name", "Codemarie Evaluation"], { cwd: workspacePath })
		await execa("git", ["config", "user.email", "codemarie@example.com"], { cwd: workspacePath })
