...
export class CodemarieAccountService {
	private static instance: CodemarieAccountService
	...
	/**
	 * Returns the singleton instance of CodemarieAccountService
	 * @returns Singleton instance of CodemarieAccountService
	 */
	public static getInstance(): CodemarieAccountService {
		if (!CodemarieAccountService.instance) {
			CodemarieAccountService.instance = new CodemarieAccountService()
		}
		return CodemarieAccountService.instance
	}

	/**
	 * Returns the base URL for the Codemarie API
	...
	async fetchMe(): Promise<UserResponse | undefined> {
		try {
			const data = await this.authenticatedRequest<UserResponse>(CODEMARIE_API_ENDPOINT.USER_INFO)
			return data
		} catch (error) {
	...
	async switchAccount(organizationId?: string): Promise<void> {
		...
		try {
			...
			const _response = await this.authenticatedRequest<string>(CODEMARIE_API_ENDPOINT.ACTIVE_ACCOUNT, {
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
				},
