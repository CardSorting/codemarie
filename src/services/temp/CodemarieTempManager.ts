/**
 * CodemarieTempManager - Manages temporary files for Codemarie with automatic cleanup.
 *
 * Simple approach:
 * - Uses a "codemarie" subdirectory inside the system temp dir (falls back to system temp if creation fails)
...
/**
 * Singleton manager for Codemarie's temporary files.
 */
class CodemarieTempManagerImpl {
	private readonly tempDir: string
	private cleanupIntervalId: NodeJS.Timeout | null = null

	constructor() {
		// Uses system temp directory with a dedicated "codemarie" subdirectory when possible:
		// macOS: /var/folders/xx/.../T/codemarie
		// Windows: C:\Users\{user}\AppData\Local\Temp\codemarie
		// Linux: /tmp/codemarie
		const baseTempDir = os.tmpdir()
		const codemarieTempDir = path.join(baseTempDir, "codemarie")

		try {
			fs.mkdirSync(codemarieTempDir, { recursive: true })
			this.tempDir = codemarieTempDir
		} catch {
	...
	/**
	 * Clean up old Codemarie temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the Codemarie temp directory
...
export const CodemarieTempManager = new CodemarieTempManagerImpl()
