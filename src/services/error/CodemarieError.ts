...
export enum CodemarieErrorType {
	Auth = "auth",
	Network = "network",
	RateLimit = "rateLimit",
	Balance = "balance",
...
export class CodemarieError extends Error {
	readonly title = "CodemarieError"
	readonly _error: ErrorDetails

	// Error details per providers:
	// Codemarie: error?.error
	// Ollama: error?.cause
	// tbc
	constructor(
		raw: any,
	...
	/**
	 * Parses a stringified error into a CodemarieError instance.
	 */
	static parse(errorStr?: string, modelId?: string): CodemarieError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return CodemarieError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a CodemarieError instance.
	 * Always returns a CodemarieError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): CodemarieError {
		try {
			// If already a CodemarieError, return it directly to prevent infinite recursion
			if (error instanceof CodemarieError) {
				return error
			}
			return new CodemarieError(JSON.parse(error), modelId, providerId)
		} catch {
			return new CodemarieError(error, modelId, providerId)
		}
	}

	public isErrorType(type: CodemarieErrorType): boolean {
		return CodemarieError.getErrorType(this) === type
	}
	 ...
	 */
	static getErrorType(err: CodemarieError): CodemarieErrorType | undefined {
		...
		if (code === "insufficient_credits" && typeof details?.current_balance === "number") {
			return CodemarieErrorType.Balance
		}
		...
		if (code === "ERR_BAD_REQUEST" || err instanceof AuthInvalidTokenError || isAuthStatus) {
			return CodemarieErrorType.Auth
		}
		...
		if (message) {
			...
			if (message?.includes(CODEMARIE_ACCOUNT_AUTH_ERROR_MESSAGE) || authErrorRegex.some((regex) => regex.test(message))) {
				return CodemarieErrorType.Auth
			}
			...
			if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
				return CodemarieErrorType.RateLimit
			}
		}
...
export class AuthNetworkError extends Error {
	constructor(
		...
		super(message)
		this.name = CodemarieErrorType.Network
	}
}
...
export class AuthInvalidTokenError extends Error {
	constructor(message: string) {
		super(message)
		this.name = CodemarieErrorType.Auth
	}
}
