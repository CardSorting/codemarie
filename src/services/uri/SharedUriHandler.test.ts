	...
	describe("handleUri", () => {
		describe("OpenRouter callback handling", () => {
			it("should successfully handle OpenRouter callback with code", async () => {
				const result = await SharedUriHandler.handleUri("vscode://codemarie.codemarie/openrouter?code=test123")
			...
			it("should return false when OpenRouter code is missing", async () => {
				const result = await SharedUriHandler.handleUri("vscode://codemarie.codemarie/openrouter")
			...
			it("should handle URL with plus signs in code parameter", async () => {
				const result = await SharedUriHandler.handleUri("vscode://codemarie.codemarie/openrouter?code=test+123+abc")
		...
		describe("Auth callback handling", () => {
			...
			it("should successfully handle auth callback without provider", async () => {
				const result = await SharedUriHandler.handleUri("vscode://codemarie.codemarie/auth?idToken=jwt123")
			...
			it("should return false when idToken is missing", async () => {
				const result = await SharedUriHandler.handleUri("vscode://codemarie.codemarie/auth?provider=google")
		...
		describe("Unknown path handling", () => {
			it("should return false for unknown paths", async () => {
				const result = await SharedUriHandler.handleUri("vscode://codemarie.codemarie/unknown?param=value")
		...
		describe("Query parameter parsing", () => {
			it("should correctly parse multiple query parameters", async () => {
				const result = await SharedUriHandler.handleUri(
					"vscode://codemarie.codemarie/auth?idToken=jwt123&provider=github&extra=param",
				)
			...
			it("should handle URL-encoded parameters", async () => {
				const result = await SharedUriHandler.handleUri(
					"vscode://codemarie.codemarie/auth?idToken=jwt%20with%20spaces&provider=google",
				)
