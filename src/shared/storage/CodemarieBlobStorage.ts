...
import { CodemarieStorage } from "./CodemarieStorage"
import { type BlobStoreSettings, type StorageAdapter } from "./types"

/**
 * S3/R2 blob storage implementation of CodemarieStorage.
 * Uses AWS S3 or Cloudflare R2 as the backend storage.
 */
export class CodemarieBlobStorage extends CodemarieStorage {
	override name = "CodemarieBlobStorage"

	private static store: CodemarieBlobStorage | null = null
	static get instance(): CodemarieBlobStorage {
		if (!CodemarieBlobStorage.store) {
			CodemarieBlobStorage.store = new CodemarieBlobStorage()
		}
		return CodemarieBlobStorage.store
	}
	...
	public init(settings?: BlobStoreSettings) {
		...
		try {
			if (!CodemarieBlobStorage.isConfigured(settings)) {
			...
			if (adapter) {
				...
				Logger.log(`[CodemarieBlobStorage] Adapter created for ${settings.adapterType}`)
			}
		} catch (error) {
			// Log but don't throw - allow startup to continue
			Logger.error("[CodemarieBlobStorage] initialization failed:", error)
		}
	}
 ...
 */
export const blobStorage = CodemarieBlobStorage.instance
