...
export interface CodemarieStorageChangeEvent {
	readonly key: string
}

export type StorageEventListener = (event: CodemarieStorageChangeEvent) => Promise<void>
 ...
 * VSCode's Memento and CodemarieSyncStorage both satisfy this interface.
 */
export interface CodemarieMemento {
 ...
 */
export abstract class CodemarieStorage {
	/**
	 * The name of the storage, used for logging purposes.
	 */
	protected name = "CodemarieStorage"
	/**
 ...
 * Unlike CodemarieStorage (string key-value, async), this stores any JSON-serializable
 * values and provides synchronous access - required for VSCode Memento compatibility.
 */

export type SyncStorageEventListener = (event: CodemarieStorageChangeEvent) => void

export abstract class CodemarieSyncStorage<T = any> {
...
/**
 * A simple in-memory implementation of CodemarieStorage using a Map.
 */
export class CodemarieInMemoryStorage extends CodemarieStorage {
	/**
