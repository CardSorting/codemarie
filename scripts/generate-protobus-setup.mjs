...
const WEBVIEW_CLIENTS_FILE = path.resolve("webview-ui/src/services/protobus-client.ts")
...
async function generateWebviewProtobusClients(protobusServices) {
	...
	static override serviceName: string = "codemarie.${serviceName}"
${rpcs.join("\n")}
}`)
	}
...
import { StreamingResponseHandler } from "@/core/controller/protobus-handler"
...
async function generateVscodeProtobusServers(protobusServices) {
	...
	for (const [serviceName, def] of Object.entries(protobusServices)) {
		...
		servers.push(`} \n`)
		serviceMap.push(`    "codemarie.${serviceName}": ${serviceName}Handlers,`)
		imports.push("")
	}
