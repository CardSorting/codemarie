...
# This installs the codemarie-core app to the user's home directory,
...
CORE_DIR=~/.codemarie/core
INSTALL_DIR=$CORE_DIR/dev-instance/
LOG_FILE=~/.codemarie/codemarie-core-service.log
...
pkill -f codemarie-core.js || true

# Detect platform name using the same logic as CodeMarieDirs.kt in the plugin.
...
echo pwd: $(pwd)
set -x
NODE_PATH=$BINARY_MODULES_DIR:./node_modules DEV_WORKSPACE_FOLDER=/tmp/ node codemarie-core.js 2>&1 | tee $LOG_FILE
